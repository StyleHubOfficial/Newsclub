...
import { SearchIcon, SettingsIcon, BookmarkIcon, MessageSquareIcon } from './icons';
    ...
    return (
        <header className="bg-brand-surface/80 backdrop-blur-md sticky top-0 z-40 border-b border-brand-primary/20 shadow-lg">
            <div className="container mx-auto px-4 py-3 flex justify-between items-center gap-4">
                {/* Logo and Branding - Hide on mobile if search is open */}
                <div className={`flex items-center gap-4 md:gap-6 flex-shrink-0 ${showMobileSearch ? 'hidden md:flex' : 'flex'}`}>
                     {/* Designer Name Plate */}
                ...
                {/* Right Side Controls */}
                <div className={`flex items-center gap-2 md:gap-3 ${showMobileSearch ? 'w-full md:w-auto' : ''}`}>
                     {/* Search Input */}
                     <form onSubmit={handleSearch} className={`${showMobileSearch ? 'flex w-full' : 'hidden'} md:block md:w-64 lg:w-96 relative`}>
                        <input
                            type="text"
                            value={query}
                        ...
                        {showMobileSearch && (
                            <button type="button" onClick={() => setShowMobileSearch(false)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-brand-text-muted md:hidden">
                                ✕
                            </button>
                        )}
                    </form>
                    ...
                    {!showMobileSearch && (
                        <button 
                            ...
                            className="md:hidden p-2 text-brand-text-muted hover:text-brand-primary transition-colors rounded-full hover:bg-brand-bg/50"
                            aria-label="Search"
                        >
                            <SearchIcon />
                        </button>
                    ...
                    {!showMobileSearch && (
                        <>
                             {/* Feedback Button */}
                             <a 
                                href="mailto:feedback@newsclub.ai?subject=News%20Club%20App%20Feedback"
                                className="p-2 text-brand-text-muted hover:text-brand-primary transition-colors rounded-full hover:bg-brand-bg/50 border border-transparent hover:border-brand-primary/30" 
