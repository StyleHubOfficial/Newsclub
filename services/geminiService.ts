...
export async function generateImageFromPrompt(prompt: string, aspectRatio: string = "1:1", model: string = 'gemini-3.1-flash-image-preview'): Promise<string> {
    try {
        const ai = getAiClient();
        ...
        if (model.startsWith('imagen-')) {
            ...
            const response = await ai.models.generateImages({
                ...
                config: {
                    numberOfImages: 1,
                    aspectRatio: aspectRatio as any, // Cast to any as the SDK types might be strict
                },
            });
            ...
            if (response.generatedImages && response.generatedImages.length > 0 && response.generatedImages[0].image && response.generatedImages[0].image.imageBytes) {
                ...
                mimeType = 'image/png'; // Default for imagen in this context
            } else {
                throw new Error("No image generated by Imagen model.");
            }
        } else {
            // Use generateContent for Gemini (Nano Banana) models
            const response = await ai.models.generateContent({
                model: model, 
                contents: [{
